...
public class NodeRoutes {
    ...
    public static HttpServer.HttpHandler nodesFromNetwork = (out, method, params) -> {
        if (method.equals("GET")) {
            UUID networkId;
            try {
    ...
    public static void init(HttpServer server) {
        server.addHandler("/nodes", NodeRoutes.allNodes);
        server.addHandler("/nodes/{uuid}", NodeRoutes.nodesFromNetwork);
    }
}
